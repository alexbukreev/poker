  ...
  return (
    <div className="mx-auto w-full max-w-[720px] rounded-md border border-border/40 p-3">
      <div className="mb-2 text-sm opacity-80">
        {scenario.tags?.join(", ")}{scenario.difficulty ? ` · ${scenario.difficulty}` : ""}
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="ml-auto flex gap-2">
          <Button onClick={onSubmit} disabled={!answer}>Submit</Button>
          <Button variant="secondary" onClick={onNext}>Next spot</Button>
        </div>
      </div>
      ...
      {verdict ? (
        <div className="mt-3 space-y-1 text-sm">
          <div>
            Result:{" "}
            {verdict.kind === "correct" ? <span className="font-semibold text-green-600">Correct</span>
            : verdict.kind === "partial" ? <span className="font-semibold text-yellow-600">Partial</span>
            : <span className="font-semibold text-red-600">Wrong</span>}
            {verdict.note ? <> · {verdict.note}</> : null}
          </div>
          {scenario.metrics ? (
            <div className="opacity-80">
              {scenario.metrics.pot_odds_threshold_pct != null && <>Threshold: {scenario.metrics.pot_odds_threshold_pct}% · </>}
              {scenario.metrics.mdf_pct != null && <>MDF: {scenario.metrics.mdf_pct}%</>}
            </div>
          ) : null}
          {scenario.comment ? <div className="opacity-90">{scenario.comment}</div> : null}
        </div>
      ) : null}
    </div>
  );
